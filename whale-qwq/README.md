...
```bash
npx @deepseek-ai/dsh plugin --profile web add whale-qwq
```
...
```bash
dsh plugin --profile web add whale-qwq && dsh web
```
...
```bash
npx @deepseek-ai/dsh plugin --profile web remove whale-qwq
```
...
`npx --registry=https://registry.npmjs.org/ @deepseek-ai/dsh plugin --profile web add whale-qwq`

**从本地检出安装（开发用）**

```bash
dsh plugin --profile web add /path/to/whale-qwq
```
